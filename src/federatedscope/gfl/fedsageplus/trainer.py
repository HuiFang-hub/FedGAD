...
class FedGenTrainer_gad(LocalGenTrainer_gad):
    def _hook_on_batch_forward(self, ctx):
        batch = ctx.data_batch.to(ctx.device)
        mask = batch['{}_mask'.format(ctx.cur_mode)]
        pred_missing, pred_feat, gad_pred = ctx.model(batch)
        pred_missing, pred_feat, gad_pred = pred_missing[mask], pred_feat[
            mask], gad_pred[mask]
        loss_gad = self.loss_function(gad_pred, self.cfg.fedsageplus.num_pred)
        loss_gad = torch.mean(loss_gad).item()*self.cfg.fedsageplus.num_pred
        ...
        ctx.loss_batch = (self.cfg.fedsageplus.a * loss_num +
                          self.cfg.fedsageplus.b * loss_feat +
                          self.cfg.fedsageplus.c *
                          loss_gad).float() / self.cfg.federate.client_num
