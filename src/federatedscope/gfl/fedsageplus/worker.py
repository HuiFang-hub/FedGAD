...
from src.federatedscope.core.data import ClientData

from src.federatedscope.gfl.trainer.nodetrainer import NodeMiniBatchTrainer
from src.federatedscope.gfl.model.fedsageplus import LocalSage_Plus, FedSage_Plus,\
    LocalSage_Plus_dgl,FedSage_Plus_dgl
from src.federatedscope.gfl.fedsageplus.utils import GraphMender, HideGraph, HideGraph_dgl
from src.federatedscope.gfl.fedsageplus.trainer import LocalGenTrainer, \
    FedGenTrainer,LocalGenTrainer_dgl
...
class FedSagePlusServer(Server):
    ...
    def check_and_move_on(self, check_eval_result=False):
        ...
        if check_eval_result: # 所有客户端参与评估
        ...
        if self.check_buffer(self.state, self.client_num) and self.state < self._cfg.fedsageplus.fedgen_epoch and \
                self.state % 2 == 0:
        ...
        if self.check_buffer( self.state, self.client_num ) and self.state < self._cfg.fedsageplus.fedgen_epoch and \
                self.state % 2 == 1 and self.grad_cnt == self.client_num * (self.client_num - 1):
        ...
        if self.check_buffer( self.state, self.client_num ) and \
                self.state == self._cfg.fedsageplus.fedgen_epoch:
            self.state += 1
            # 为每个客户端设置 Clf_trainer
            self.comm_manager.send(
                Message(msg_type='setup',
                        ...
                        state=self.state))
        # 开始评估
        if self.check_buffer(self.state, minimal_number, check_eval_result ) and \
                self.state >= self._cfg.fedsageplus.fedgen_epoch:
...
class FedSagePlusClient(Client):
    def __init__(self,
        ...
        self.data = data
        if config.data.dgl:
            self.hide_data = HideGraph_dgl(self._cfg.fedsageplus.hide_portion)(data['data'])
        else:
            self.hide_data = HideGraph(self._cfg.fedsageplus.hide_portion)( data['data'])
        ...
        if config.data.dgl:
            in_channels = data['data'].ndata['feat'].shape[-1]
        else:
            in_channels =data['data'].x.shape[-1]

        self.gen = LocalSage_Plus(in_channels,
...
class FedSagePlusClient_dgl(Client):
    def __init__(self,
                 ID=-1,
                 server_id=None,
                 state=-1,
        ...
        self.device = device
        self.sage_batch_size = 64
        if config.dataloader.type == 'dgl':
            in_channels = data['data'].ndata['feat'].shape[-1]
        else:
            in_channels =data['data'].x.shape[-1]
        if config.dataloader.type == 'dgl':
            self.gen = LocalSage_Plus_dgl(in_channels,
                                      self._cfg.model.out_channels,
                                      hidden=self._cfg.model.hidden,
                                      gen_hidden=self._cfg.fedsageplus.gen_hidden,
                                      dropout=self._cfg.model.dropout,
        ...
        self.gad = model
        if config.dataloader.type == 'dgl':
            self.trainer_loc = LocalGenTrainer_dgl(self.gen,
                                              self.hide_data,
                                              self.device,
                                              self._cfg,
                                              monitor=self._monitor)
    ...
    def callback_funcs_for_local_pre_train(self, message: Message):
        # 客户端完成本地预训练,并向服务器发送信息： 轮数、fedgen 的参数和嵌入以及隐藏数据集的缺失值数量
        round, sender, _ = message.state, message.sender, message.content
        # Local pre-train
        logger.info(f'\tClient #{self.ID} pre-train start...')
        for i in range(self._cfg.fedsageplus.loc_epoch):
    ...
    def callback_funcs_for_gen_para(self, message: Message):
        # 客户端从服务器获取信息的回合数、发送者、内容
        round, sender, content = message.state, message.sender, message.content
        gen_para, embedding, label, ID = content
        # 使用 FedGenTrainer 计算模型参数梯度
        gen_grad = self.trainer_fedgen.cal_grad(self.data['data'], gen_para,
                                                embedding, label)
        self.state = round
        # 将计算出的梯度发送给服务端
    ...
    def callback_funcs_for_setup_fedsage(self, message: Message):
        ...
        self._cfg.merge_from_list(
            ['dataloader.batch_size', self.sage_batch_size])
        self.trainer_clf = NodeMiniBatchTrainer(self.gad,
                                                fill_dataloader,
                                                self.device,
                                                self._cfg,
                                                monitor=self._monitor)
...
class FedSagePlusClient_pyg(Client):
    def __init__(self,
                 ID=-1,
                 server_id=None,
                 state=-1,
