...
class Trainer(BaseTrainer):
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        ...
        if self.ctx.cfg.model.type.lower() not in ['cola','anemone']:
    ...
    def train_gad(self, target_data_split_name="train", hooks_set=None):
        hooks_set = hooks_set or self.hooks_in_train
    ...
    def _run_routine(self, mode, hooks_set, dataset_name=None):
        ...
        for hook in hooks_set["on_fit_end"]:
            hook(self.ctx)
        # if self.ctx.cfg.model.type == 'cola':
        #     return self.ctx.num_samples_total
        # else:
        return self.ctx.num_samples #self.ctx.num_samples_total

    @lifecycle(LIFECYCLE.EPOCH)
    def _run_epoch(self, hooks_set):
        if self.ctx.cur_mode == "train":
            epoch_num = getattr(self.ctx, f"num_{self.ctx.cur_split}_epoch")
    ...
    def _run_batch(self, hooks_set):
        # print(self.ctx.batch_num)
        test2 = getattr(self.ctx, f"num_{self.ctx.cur_split}_batch")
        batch_num = self.ctx.batch_num if hasattr(self.ctx, 'batch_num') else getattr(self.ctx, f"num_{self.ctx.cur_split}_batch")
        for batch_i in range(batch_num):
            ...
            for hook in hooks_set["on_batch_end"]:
                hook(self.ctx)
            if self.ctx.cur_mode in [MODE.TRAIN,
                                     MODE.FINETUNE] and self.ctx.cur_epoch_i == self.ctx.num_train_epoch - 1:
