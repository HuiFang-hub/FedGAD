...
from src.federatedscope.core.auxiliaries.criterion_builder import get_criterion
from src.federatedscope.core.auxiliaries.model_builder import \
    get_trainable_para_names
from src.federatedscope.core.auxiliaries.regularizer_builder import get_regularizer
from src.federatedscope.core.trainers.enums import MODE
...
class Context(LifecycleDict):
    ...
    Note:
        ...
        [tutorial](https://src.federatedscope.io/docs/trainer/).
    ...
    def __init__(self, model, cfg, data=None, device=None):
        ...
        if self.cfg.backend == 'torch':
            ...
            self.criterion = get_criterion(self.cfg.criterion.type,
                                           self.device)
            self.model_criterion = get_criterion(self.cfg.criterion.model_type,
                                           self.device)
