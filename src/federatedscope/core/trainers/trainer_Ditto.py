...
from src.federatedscope.core.optimizer import wrap_regularized_optimizer
from src.federatedscope.core.trainers.utils import calculate_batch_epoch_num
...
def init_Ditto_ctx(base_trainer):
    ...
    `local_models` acts as personalized model will be optimized with
    ...
    ctx.local_models = copy.deepcopy(ctx.model)  # the personalized model
    ctx.models = [ctx.local_models, ctx.global_model]
...
def _hook_on_fit_start_set_regularized_para(ctx):
    ...
    Note:
      The modified attributes and according operations are shown below:
        ...
        to ``train`` mode
        ``ctx.local_models``                 Move to ``ctx.device`` and set \
        to ``train`` mode
    ...
    ctx.local_models.to(ctx.device)
    ctx.global_model.train()
    ctx.local_models.train()
    ...
    ctx.optimizer_for_local_model = get_optimizer(ctx.local_models,
                                                  **ctx.cfg.train.optimizer)
...
def _hook_on_batch_start_switch_model(ctx):
    ...
    Note:
      The modified attributes and according operations are shown below:
        ...
        ``ctx.model``                       Set to ``ctx.local_models`` or \
        ``ctx.global_model``
...
def _hook_on_fit_start_switch_local_model(ctx):
    ...
    Note:
      The modified attributes and according operations are shown below:
        ...
        Attribute                           Operation
        ==================================  ===========================
        ``ctx.model``                       Set to ``ctx.local_models`` and \
        set to ``eval`` mode
        ==================================  ===========================
    """
    ctx.model = ctx.local_models
...
def _hook_on_fit_end_free_cuda(ctx):
    ...
    ctx.local_models.to(torch.device("cpu"))
