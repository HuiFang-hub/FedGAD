...
from src.federatedscope.core.auxiliaries.utils import param2tensor, \
    merge_param_dict
from src.federatedscope.core.monitors.monitor import Monitor
...
class GeneralTorchTrainer(Trainer):
    ...
    def update(self, model_parameters, strict=False):
        ...
        test = self.ctx.model.state_dict().copy()
        test2 = self._param_filter(model_parameters)
    ...
    def _hook_on_fit_start_init(self, ctx):
        ...
        #  and data split, so the number of batch and epoch should beR
    ...
    def _hook_on_epoch_start(self, ctx):
        ...
        if ctx.get("{}_loader".format(ctx.cur_split)) is None:
            if ctx.get("{}_data".format(ctx.cur_split)) is None:
                loader = get_dataloader(ctx.data['data'],
