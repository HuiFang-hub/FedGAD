...
from src.federatedscope.core.auxiliaries.logging import logline_2_wandb_dict
from src.federatedscope.core.monitors.metric_calculator import MetricCalculator
...
class Monitor(object):
    ...
    def finish_fed_runner(self, fl_mode=None):
        ...
        if self.use_wandb and not self.wandb_online_track:
            ...
            from src.federatedscope.core.auxiliaries.logging import \
    ...
    def format_eval_res(self,
                ...
                if not role.lower().startswith('server') or form == 'raw':
                    ...
                    for key in results.keys():
                        ...
                        if key in [
                            ...
                            new_results[key] = np.mean(new_results[key])
                        elif key in [f'{dataset_name}_fpr',f'{dataset_name}_tpr',]:
                            continue
                        else:
        ...
        with open(os.path.join(self.outdir, "eval_results_raw.log"),
                  "a") as outfile:
    ...
    def calc_model_metric(self, last_model, local_updated_models, rnd):
        ...
        for metric in self.cfg.eval.monitoring:
            ...
            calc_metric = getattr(
                import_module(
                    'src.federatedscope.core.monitors.metric_calculator'),
                func_name)
