...
from src.federatedscope.core.auxiliaries.metric_builder import get_metric
...
class MetricCalculator(object):
    ...
    def eval(self, ctx):
        ...
        for metric, (func, _) in self.eval_metric.items():
            results["{}_{}".format(ctx.cur_split,metric)] = func(ctx=ctx,
                                                   y_true=y_true,
                                                   y_pred=y_pred,
                                                   y_prob=y_prob,
                                                   metric=metric)
    ...
    def _check_and_parse(self, ctx):
        ...
        if ctx.get('ys_pred', None) is not None:
            ...
            if 'regression' in ctx.cfg.model.task.lower():
                y_pred = None
            if 'fedsage' in ctx.cfg.federate.method:
...
def eval_roc_auc(y_true, y_prob, **kwargs):
    ...
    # for i in range(y_true.shape[1]):
    #     # AUC is only defined when there is at least one positive data.
    #     test = np.sum(y_true[:, i] == 1)
    #     test2 = np.sum(y_true[:, i] == 0)
    #
    #     if np.sum(y_true[:, i] == 1) > 0 and np.sum(y_true[:, i] == 0) > 0:
    #         # ignore nan values
    #         is_labeled = y_true[:, i] == y_true[:, i]
    #         # TODO: handle missing label classes
    #         test = softmax(y_prob[is_labeled, :, i])
    #         test2= y_true[is_labeled, i]
    #         rocauc_list.append(
    #             roc_auc_score(y_true[is_labeled, i],softmax(y_prob[is_labeled, :, i]),multi_class='ovr'))
    #
...
def eval_recall(y_true,y_prob,**kwargs):
    recall = recall_score(y_true=y_true, y_pred=y_prob)
    return recall
