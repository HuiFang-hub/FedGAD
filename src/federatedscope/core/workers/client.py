...
from src.federatedscope.core.auxiliaries.utils import merge_dict_of_results, \
    calculate_time_cost
from src.federatedscope.core.workers.base_client import BaseClient
import wandb
        ...
        if 'ss' in message.msg_type:
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode:
                ...
                if self.early_stopper.early_stopped and \
                    ...
                    self._monitor.local_converged()
                sample_size, model_para_all, results = self.trainer.train_gad()
                ...
                if self._cfg.wandb.use:
                    wandb.log({"Client {} Results".format(self.ID): results})
                # wandb.log(train_log_res)
