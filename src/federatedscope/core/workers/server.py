...
class Server(BaseServer):
    ...
    Attributes:
        ...
        comm_manager: manager for communication, \
            see ``src.federatedscope.core.communication``
    """
    def __init__(self,
                 ID=-1,
                 state=0,
        ...
        if self._cfg.federate.make_global_eval:
            ...
            for i in range(self.model_num):
                ...
                if self._cfg.wandb.use:
                    wandb.log({"Server # Results".format(self.ID): formatted_eval_res['Results_raw']})

                # if self.state< self._cfg.federate.total_round_num+self._cfg.fedsageplus.fedgen_epoch:
                #     # formatted_eval_res[fpr, tpr] = 0, 0
                #     formatted_eval_res['Results_raw']['test_fpr'] = 0
                #     formatted_eval_res['Results_raw']['test_tpr'] = 0
                    # print("test")
