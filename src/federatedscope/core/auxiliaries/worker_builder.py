...
from src.federatedscope.core.configs import constants
from src.federatedscope.core.workers import Server, Client
import src.federatedscope.register as register

logger = logging.getLogger(__name__)

try:
    from src.federatedscope.contrib.worker import *
except ImportError as error:
    logger.warning(
        f'{error} in `src.federatedscope.contrib.worker`, some modules are not '
        f'available.')
...
def get_client_cls(cfg):
    ...
    Args:
        cfg: configurations for FL, see ``src.federatedscope.core.configs``
    ...
    if cfg.hpo.fedex.use:
        from src.federatedscope.autotune.fedex import FedExClient
        return FedExClient

    if cfg.vertical.use:
        if cfg.vertical.algo == 'lr':
            from src.federatedscope.vertical_fl.worker import vFLClient
            return vFLClient
        elif cfg.vertical.algo == 'xgb':
            from src.federatedscope.vertical_fl.xgb_base.worker import XGBClient
            return XGBClient
        else:
            raise ValueError(f'No client class for {cfg.vertical.algo}')

    if cfg.data.type.lower() == 'hetero_nlp_tasks':
        from src.federatedscope.nlp.hetero_tasks.worker import ATCClient
        return ATCClient
    ...
    if client_type == 'fedsageplus':
        from src.federatedscope.gfl.fedsageplus.worker import FedSagePlusClient,FedSagePlusClient_dgl,FedSagePlusClient_pyg
        if cfg.model.type.lower() in ['cola','anemone']:
            if cfg.dataloader.type == 'dgl':
                client_class = FedSagePlusClient_dgl
            else:
                client_class = FedSagePlusClient_pyg
        else:
            client_class = FedSagePlusClient

    elif client_type == 'gcflplus':
        from src.federatedscope.gfl.gcflplus.worker import GCFLPlusClient
        client_class = GCFLPlusClient
    elif client_type == 'fedgc':
        from src.federatedscope.cl.fedgc.client import GlobalContrastFLClient
        client_class = GlobalContrastFLClient
    elif client_type == 'fedsagegod':
        from src.federatedscope.gfl.fedsageGod.worker import FedSageGodClient
        client_class = FedSageGodClient
    else:
        client_class = Client
    ...
    if client_atk_type == 'gradascent':
        from src.federatedscope.attack.worker_as_attacker.active_client import \
...
def get_server_cls(cfg):
    ...
    if cfg.hpo.fedex.use:
        from src.federatedscope.autotune.fedex import FedExServer
        return FedExServer

    if cfg.attack.attack_method.lower() in ['dlg', 'ig']:
        from src.federatedscope.attack.worker_as_attacker.server_attacker import\
            PassiveServer
        ...
        from src.federatedscope.attack.worker_as_attacker.server_attacker \
            import BackdoorServer
        return BackdoorServer

    if cfg.vertical.use:
        if cfg.vertical.algo == 'lr':
            from src.federatedscope.vertical_fl.worker import vFLServer
            return vFLServer
        elif cfg.vertical.algo == 'xgb':
            from src.federatedscope.vertical_fl.xgb_base.worker import XGBServer
            return XGBServer
        else:
            raise ValueError(f'No server class for {cfg.vertical.algo}')

    if cfg.data.type.lower() == 'hetero_nlp_tasks':
        from src.federatedscope.nlp.hetero_tasks.worker import ATCServer
        return ATCServer
    ...
    if server_type == 'fedsageplus':
        from src.federatedscope.gfl.fedsageplus.worker import FedSagePlusServer
        server_class = FedSagePlusServer
    elif server_type == 'gcflplus':
        from src.federatedscope.gfl.gcflplus.worker import GCFLPlusServer
        server_class = GCFLPlusServer
    elif server_type == 'fedgc':
        from src.federatedscope.cl.fedgc.server import GlobalContrastFLServer
        server_class = GlobalContrastFLServer
    elif server_type == 'fedsagegod':
        from src.federatedscope.gfl.fedsageGod.worker import FedSageGodServer
        server_class = FedSageGodServer
    # elif server_type == 'fedsagegod':
    #     from src.federatedscope.gfl.fedsageGod.worker import FedSageGodServer
    #     server_class = FedSageGodServer
    else:
