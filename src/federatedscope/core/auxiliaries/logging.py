...
def update_logger(cfg, clear_before_add=False):
    ...
    # 创建自定义的格式化器
    class NoLimitFormatter(logging.Formatter):
        def format(self, record):
            # 使用record.msg代替record.message，确保消息没有被截断
            if isinstance(record.msg, dict):
                record.msg = {k: str(v) if isinstance(v, np.ndarray) else v for k, v in record.msg.items()}
            return super().format(record)

    # 创建文件处理器并设置格式化器
    fh = logging.FileHandler(os.path.join(cfg.outdir, 'exp_print.log'))
    fh.setLevel(logging.DEBUG)
    logger_formatter = NoLimitFormatter("%(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s")
