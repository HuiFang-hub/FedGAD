...
from src.federatedscope.core.trainers import Trainer

logger = logging.getLogger(__name__)

try:
    from src.federatedscope.contrib.trainer import *
except ImportError as error:
    logger.warning(
        f'{error} in `src.federatedscope.contrib.trainer`, some modules are not '
        f'available.')
...
def get_trainer(model=None,
    ...
    Arguments:
        ...
        is an attacker
        monitor: an instance of ``src.federatedscope.core.monitors.Monitor`` to \
    ...
    Note:
      The key-value pairs of ``cfg.trainer.type`` and trainers:
        ...
        ``mftrainer``                       \
        ``src.federatedscope.mf.trainer.MFTrainer``
        ``mytorchtrainer``                  \
    ...
    if config.trainer.type == 'general':
        if config.backend == 'torch':
            ...
            from src.federatedscope.core.trainers import GeneralTFTrainer
        ...
        if config.trainer.type.lower() in ['cvtrainer']:
            dict_path = "src.federatedscope.cv.trainer.trainer"
        elif config.trainer.type.lower() in ['nlptrainer']:
            dict_path = "src.federatedscope.nlp.trainer.trainer"
        elif config.trainer.type.lower() in ['cltrainer', 'lptrainer']:
            dict_path = "src.federatedscope.cl.trainer.trainer"
        elif config.trainer.type.lower() in [
                'graphminibatch_trainer',
        ]:
            dict_path = "src.federatedscope.gfl.trainer.graphtrainer"
        elif config.trainer.type.lower() in [
                'linkfullbatch_trainer', 'linkminibatch_trainer'
        ]:
            dict_path = "src.federatedscope.gfl.trainer.linktrainer"
        elif config.trainer.type.lower() in [
                'nodefullbatch_trainer', 'nodeminibatch_trainer'
        ]:
            dict_path = "src.federatedscope.gfl.trainer.nodetrainer"
        elif config.trainer.type.lower() in [
                'flitplustrainer', 'flittrainer', 'fedvattrainer',
                'fedfocaltrainer'
        ]:
            dict_path = "src.federatedscope.gfl.flitplus.trainer"
        elif config.trainer.type.lower() in ['mftrainer']:
            dict_path = "src.federatedscope.mf.trainer.trainer"
        elif config.trainer.type.lower() in ['atc_trainer']:
            dict_path = "src.federatedscope.nlp.hetero_tasks.trainer"
        else:
            raise ValueError
        ...
        from src.federatedscope.vertical_fl.trainer.utils import \
        ...
        trainer = get_vertical_trainer(config=config,
                                       ...
                                       device=device,
                                       monitor=monitor)
    elif config.trainer.type.lower() in ['fedgod']:
        from src.federatedscope.gfl.fedgod.trainer import fedgod_trainer
        trainer = fedgod_trainer(model=model,
                              data=data,
                              device=device,
                              config=config,
                              only_for_eval=only_for_eval,
                              monitor=monitor)
    elif config.trainer.type.lower() in ['fedanemone']:
        from src.federatedscope.gfl.fedAnemone.trainer import fedanemone_trainer
        trainer = fedanemone_trainer(model=model,
                              data=data,
                              device=device,
                              config=config,
                              only_for_eval=only_for_eval,
    ...
    if not isinstance(trainer, Trainer):
        logger.warning(f'Hook-like plug-in functions cannot be enabled when '
                       ...
                       f'inheriting from '
                       f'`src.federatedscope.core.trainers.Trainer` instead.')
        return trainer

    # differential privacy plug-in
    if config.nbafl.use:
        from src.federatedscope.core.trainers import wrap_nbafl_trainer
        trainer = wrap_nbafl_trainer(trainer)
    if config.sgdmf.use:
        from src.federatedscope.mf.trainer import wrap_MFTrainer
        trainer = wrap_MFTrainer(trainer)

    # personalization plug-in
    if config.federate.method.lower() == "pfedme":
        from src.federatedscope.core.trainers import wrap_pFedMeTrainer
        ...
        from src.federatedscope.core.trainers import wrap_DittoTrainer
        # wrap style: instance a (class A) -> instance a (class A)
        trainer = wrap_DittoTrainer(trainer)
    elif config.federate.method.lower() == "fedem":
        from src.federatedscope.core.trainers import FedEMTrainer
    ...
    if 'backdoor' in config.attack.attack_method:
        from src.federatedscope.attack.trainer import wrap_benignTrainer
    ...
    if is_attacker:
        ...
        from src.federatedscope.attack.auxiliary.attack_trainer_builder \
    ...
    if config.fedprox.use:
        from src.federatedscope.core.trainers import wrap_fedprox_trainer
