...
try:
    from src.federatedscope.contrib.model import *
except ImportError as error:
    logger.warning(
        f'{error} in `src.federatedscope.contrib.model`, some modules are not '
        f'available.')


def get_shape_from_data(data, data_type,model_config, backend='torch'):
    """
    ...
    if model_config.type.lower() in ['vmfnet', 'hmfnet']:
        ...
        return None
    elif model_config.type.lower() in ['cola']:
        if data_type == 'dgl':
            g = data['data']
            return g.ndata['feat'].shape[1],g.num_nodes()
        else:
            return data['data'].x.shape
    ...
    if isinstance(data_representative, dict):
        ...
        if issubclass(type(data_representative), torch.utils.data.DataLoader):
            ...
            try:
                ...
                #     return x[0].shape
                # return x.shape
                x = data_representative.x
                return  x.shape
            except:
                raise TypeError('Unsupported data type.')
        # if isinstance(data_representative, torch.utils.data.Dataset):
        #     x = data_representative[0].x
            # return x.shape
...
def get_model(model_config, data_type,device,local_data=None, backend='torch'):
    """
    ...
    if local_data is not None:
        input_shape = get_shape_from_data(local_data,data_type, model_config, backend)
    else:
    ...
    for func in register.model_dict.values():
        model = func(model_config, input_shape,device)
        if model is not None:
            return model

    if model_config.type.lower() == 'lr':
        if backend == 'torch':
            from src.federatedscope.core.lr import LogisticRegression
            model = LogisticRegression(in_channels=input_shape[-1],
                                       class_num=model_config.out_channels)
        elif backend == 'tensorflow':
            from src.federatedscope.cross_backends import LogisticRegression
            model = LogisticRegression(in_channels=input_shape[-1],
                                       class_num=1,
        ...
        from src.federatedscope.nlp.model import get_rnn
        model = get_rnn(model_config, input_shape)
    elif model_config.type.lower().endswith('transformers'):
        from src.federatedscope.nlp.model import get_transformer
        model = get_transformer(model_config, input_shape)
    elif model_config.type.lower() in [
            'gcn', 'sage', 'gpr', 'gat', 'gin', 'mpnn'
    ]:
        from src.federatedscope.gfl.model import get_gnn
        model = get_gnn(model_config, input_shape)
    elif model_config.type.lower() in ['vmfnet', 'hmfnet']:
        from src.federatedscope.mf.model.model_builder import get_mfnet
        model = get_mfnet(model_config, input_shape)
    elif model_config.type.lower() in ['xgb_tree']:
        from src.federatedscope.vertical_fl.model.model_builder import \
            get_tree_model
        model = get_tree_model(model_config)
    elif model_config.type.lower() in ['atc_model']:
        from src.federatedscope.nlp.hetero_tasks.model import ATCModel
        model = ATCModel(model_config)
    # elif model_config.type.lower() in ['cola','one','done','aagnn']:
    #     from src.dgld.models.model import DGLDmodel
    #     model = DGLDmodel(model_config)
    else:
