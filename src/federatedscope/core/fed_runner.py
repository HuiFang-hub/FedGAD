...
from src.federatedscope.core.gpu_manager import GPUManager
from src.federatedscope.core.auxiliaries.model_builder import get_model
from src.federatedscope.core.auxiliaries.utils import get_resource_info
from src.federatedscope.core.auxiliaries.feat_engr_builder import \
...
class BaseRunner(object):
    ...
    Args:
        ...
        src.federatedscope.core.auxiliaries.data_builder .
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        if self.cfg.nbafl.use:
            from src.federatedscope.core.trainers.trainer_nbafl import \
                wrap_nbafl_server
            wrap_nbafl_server(server)
        if self.cfg.vertical.use:
            from src.federatedscope.vertical_fl.utils import wrap_vertical_server
    ...
    def _setup_client(self,
        ...
        client = self.client_class(ID=client_id,
                                   ...
                                   model=client_model
                                    or get_model(client_specific_config.model,
                                                 self.cfg.dataloader.type,self.cfg.device,
                                                client_data,
        ...
        if self.cfg.vertical.use:
            from src.federatedscope.vertical_fl.utils import wrap_vertical_client
...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        self._shared_client_model = get_model(
            self.cfg.model,self.cfg.dataloader.type,self.cfg.device, self.data[1], backend=self.cfg.backend
    ...
    def _get_server_args(self, resource_info=None, client_resource_info=None):
        if self.server_id in self.data:
            ...
            model = get_model(self.cfg.model,self.cfg.dataloader.type,self.cfg.device,
                              server_data,
                              backend=self.cfg.backend)
        else:
            server_data = None
            ...
            model = get_model( self.cfg.model, self.cfg.dataloader.type,self.cfg.device,
                               data_representative, backend=self.cfg.backend)
    ...
    def _run_simulation(self):
        ...
        while True:
            if len(self.shared_comm_queue) > 0: # 从client端收集信息存在server_msg_cache
                msg = self.shared_comm_queue.popleft()
                # 纵向联邦
                if not self.cfg.vertical.use and msg.receiver == [ self.server_id]:
                    ...
                    self._handle_msg(msg)
                    # print(f"0,state={msg.state}")
            elif len(server_msg_cache) > 0: # 处理服务器中的信息
...
class FedRunner(object):
    ...
    def _setup_for_standalone(self):
        ...
        # shape
        self._shared_client_model = get_model(
            self.cfg.model, self.data[1],self.cfg.device, backend=self.cfg.backend
        ) if self.cfg.federate.share_local_model else None
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        if self.mode == 'standalone':
            if self.server_id in self.data:
                ...
                model = get_model(
                    self.cfg.model,
                    data_representative,self.cfg.device,
                    backend=self.cfg.backend
                )  # get the model according to client's data if the server
                # does not own data
            kw = {
