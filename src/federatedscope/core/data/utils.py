...
def load_dataset(config, client_cfgs=None):
    ...
    if config.data.type.lower() == 'toy':
        ...
        from src.federatedscope.gfl.dataloader import load_nodelevel_dataset
        dataset, modified_config = load_nodelevel_dataset(config)
    elif config.data.type.lower() in ['ciao', 'epinions', 'fb15k-237', 'wn18']:
        from src.federatedscope.gfl.dataloader import load_linklevel_dataset
        ...
        from src.federatedscope.gfl.dataloader import load_graphlevel_dataset
        dataset, modified_config = load_graphlevel_dataset(config)
    elif config.data.type.lower() in [
            'synthetic_vfl_data', 'adult', 'abalone', 'credit', 'blog'
    ]:
        from src.federatedscope.vertical_fl.dataloader import load_vertical_data
    ...
    # elif config.data.type.lower() in ['core_inj']:
    #     from src.federatedscope.contrib.data.core_inj import load_external_data
    else:
...
def load_external_data(config=None):
    ...
    Args:
        config: `CN` from `src.federatedscope/core/configs/config.py`

    Returns:
        (data, modified_config): tuple of ML split dataset, \
        and `CN` from `src.federatedscope/core/configs/config.py`, \
    ...
    def load_torchtext_data(name, splits=None, config=None):
        ...
        from src.federatedscope.nlp.dataset.utils import label_to_index
...
def convert_data_mode(data, config):
    ...
    Args:
        ...
        config: configuration of FL course, see `src.federatedscope.core.configs`
