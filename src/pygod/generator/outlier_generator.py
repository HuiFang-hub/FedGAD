...
def gen_structural_outliers(data, m, n, p=0, random_state=None,outlierIdx = None):
    ...
    if len(outlierIdx):
        ...
        candidate_set = node_set.difference(set(outlierIdx))
        outlier_idx = np.random.choice(list(candidate_set), size=m * n, replace=False)
    else:
    ...
    return data, y_outlier,outlier_idx
